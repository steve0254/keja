...
import { chipTypes, computeTrendingNeighborhoods } from "@/lib/listings";
  ...
  head: () => ({
    meta: [
      { title: "Keja — Find a home, live" },
      {
        name: "description",
        content:
          "Real-time rental availability. Discover vacant homes in your city and book viewings in minutes.",
      },
    ],
  }),
  component: Home,
...
function Home() {
  ...
  const trendingHoods = useMemo(() => computeTrendingNeighborhoods(listings), [listings]);
  const justListed = useMemo(
    () => (activeType === "All" ? listings : listings.filter((l) => l.type === activeType)),
    [listings, activeType],
  );
  ...
  return (
    <AppShell>
        ...
        <div className="flex items-center justify-between">
          ...
          {user ? (
            <button
              ...
              className="press glass relative flex h-11 w-11 items-center justify-center rounded-2xl"
            >
            ...
            <Link
              to="/auth"
              className="press rounded-2xl bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground shadow-pop"
            >
              Sign in
            </Link>
          )}
      ...
      <section className="animate-fade-up mt-6 px-5">
        <Link
          to="/search"
          className="press flex items-center gap-3 rounded-3xl bg-card p-4 shadow-soft"
        >
        ...
        >
          ...
          <span className="flex-1">
            ...
            <span className="mt-0.5 block text-sm font-medium">
              {listings.length} homes vacant near you
            </span>
          </span>
      ...
      <section className="animate-fade-up mt-8">
        ...
        {isLoading ? (
          ...
          <p className="px-5 text-sm text-muted-foreground">
            No {activeType.toLowerCase()} listings right now — try another type.
          </p>
        ) : (
          <div className="flex gap-4 overflow-x-auto px-5 pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            {justListed.map((l) => (
        ...
        <div className="grid grid-cols-2 gap-3">
          {trendingHoods.map((n) => (
            <Link
              ...
              to="/search"
              search={{ q: n.name }}
              className="press rise rounded-2xl bg-card p-4 text-left shadow-soft"
            >
  ...
  return (
    <div className="mx-5 rounded-3xl border border-dashed border-border bg-card p-6 text-center">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Be the first — landlords can publish a home in under a minute.
      </p>
      <Link
        to="/add"
        className="press mt-4 inline-flex rounded-2xl bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground"
      >
        Add a vacancy
      </Link>
    </div>
